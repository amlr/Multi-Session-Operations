...
switch(_operation) do {
	...
	case "houses": {
		if(isNil "_args") then {
			_args = _logic getVariable ["houses", []];		
		...
		if ([_logic, "debug"] call MSO_fnc_CQB) then {
			...
			{
				[format[_mtemplate, _x], getPosATL _x, "Icon", [1,1], "TYPE:", "Dot", "COLOR:", _logic getVariable ["debugColor","ColorGreen"], "TEXT:", _logic getVariable ["debugPrefix","CQB"], "GLOBAL"] call CBA_fnc_createMarker;
	...
	case "addHouse": {
		if(!isNil "_args") then {
			...
			[_logic,"houses",[_args],true,true] call BIS_fnc_variableSpaceAdd;
			if ([_logic, "debug"] call MSO_fnc_CQB) then {
				format["CQB Population: Adding house %1...", _args] call MSO_fnc_logger;
				[format[_mtemplate, _args], getPosATL _args, "Icon", [1,1], "TYPE:", "Dot", "COLOR:", _logic getVariable ["debugColor","ColorGreen"], "TEXT:", _logic getVariable ["debugPrefix","CQB"], "GLOBAL"] call CBA_fnc_createMarker;
			};
