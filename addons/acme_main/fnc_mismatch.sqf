...
#define __det (parseText (format ["%1",localize "STR_ACME_VERSION_DETECT"]))
...
#define __update (parseText (format ["%1", localize "STR_ACME_VERSION_UPDATE"]))

private "_i";
PARAMS_3(_serverVersion,_localVersion,_player);

if (isNil "ACME_MISMATCH") then {ACME_MISMATCH = true}; // Allow an override set by developer during testing or else.

#define CTRL(A) ((findDisplay 114111) displayCtrl A)

_i = 0;
while {ACME_MISMATCH} do {
	waitUntil {!dialog};
	if (_i > 5) exitWith {endMission "END6"}; // disconnect after more than 5 repeats.
	INC(_i);

	createDialog "ACME_VERSION_MISMATCH";
	CTRL(114112) ctrlSetText (localize "STR_ACME_VERSION_WARNING");
