...
#define __cfgpath configFile >> "cfgSettings" >> "ACME" >> "acme_server_settings"
...
0 spawn { //this spawns on dedi server when mission starts
	...
	_checkall = getNumber(__cfgpath >> "check_all_acme_pbos");
	...
	_fVerifyFiles = {
		...
		if (_checkAll == 1) then {
			_listAll = getArray (configFile >> "ACME_Config" >> "acme_pbos" >> "acme_pbos");
	...
	_logic = "ACME_LogicDummy" createvehicle __dummyPosSet;
