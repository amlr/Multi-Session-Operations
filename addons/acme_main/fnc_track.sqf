...
_tracker = "ACME_LogicDummy" createVehicleLocal _pos;
_particle = [_tracker, _color1 select 0] call ACME_fnc_particle;

ADD(GVAR(marker_debug),1);
_marker = createMarkerLocal [format["acme_debug_mrk2_%1", GVAR(marker_debug)], _pos];
...
while {alive _object} do {
	...
	if ((_pos select 0) != 0 && {(_pos select 1) != 0} && {(_pos select 2) != 0}) then {
		...
		if (time > _waitUntil) then {
			...
			_o = "ACME_LogicDummy" createVehicleLocal _pos;
			_o setPosASL _pos;
			PUSH(_objects,_o);
			_p = [_o, _color2 select 0] call ACME_fnc_particle;
			PUSH(_particles,_p);
			ADD(GVAR(marker_debug),1);
			_m = createMarkerLocal [format["acme_debug_mrk2_%1", GVAR(marker_debug)], _pos];
