...
#include "\x\acme\addons\main\script_common.hpp"

if (!isNil "ACME_ABOUT_RUN") exitWith {ACME_ABOUT_RUN = false;};
ACME_ABOUT_RUN = true;
...
_pkeynam = { //local function
	...
	_keystrg = "^";
	{
		...
		_knam = "^";
		{_k = _x; if (!_mod || (_k != (localize "STR_ACME_KN_LEFT") && _k != (localize "STR_ACME_KN_RIGHT"))) then {_knam = _knam + " " + _k}} forEach _knaml;
...
_pkeys = {
	...
	for "_i" from 0 to (count _cEvents) - 1 do {
		...
		if (isNumber ((_cSys select 0)/"key")) then {
			...
			_tS = "^";
			{if (_x != "ACME" && _x != "sys") then {_tS = _tS + " " + _x}} forEach _aSys;
...
if (_pages > 0) then {
	//Dialog
	createDialog "ACME_ABOUT_DLG";
	_disp = uiNamespace getVariable "ACME_ABOUT_DLG";
	...
	_input = { //local function
		_nesc = isNil "ACME_ABOUT_STP";
		if (_pages == 1) then {ACME_ABOUT_INC = 0}; //ignore special control
		_unset = (ACME_ABOUT_INC == 0) && ACME_ABOUT_RUN;
