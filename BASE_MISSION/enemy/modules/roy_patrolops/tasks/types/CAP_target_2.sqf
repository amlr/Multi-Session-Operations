...
objective_intel_KILLED = {
	if(side _this == (SIDE_B select 0) && random 3 < 1) then {
		_this addEventHandler ["Killed",{
			...
			_ev setPosATL _pos;
			_ev spawn {
				while{alive CAPT2_target && {isPlayer _x && side _x == (SIDE_A select 0)} count nearestObjects[position _this,["All"],3] == 0} do {sleep 2};
...
while { alive CAPT2_target && CAPT2_target distance (getMarkerPos format["return_point_%1",(SIDE_A select 0)]) > 10 } do { sleep 5 };

mps_civilian_intel = []; publicVariable "mps_civilian_intel";

if(_random_scenario > 0 && alive CAPT2_target) then {
