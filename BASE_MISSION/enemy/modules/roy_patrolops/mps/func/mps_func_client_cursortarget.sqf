...
// LOGISTICS CONTAINERS
			if ({_cursTarget isKindOf (_x select 0)} count mps_transportable_containers > 0) then { _load_container = _cursTarget addaction [format[  "Load %1",_type],(mps_path+"action\mps_logistics_load_container.sqf"),[],-1,true,true,"",_condition_load]; };
...
// Repair Action
			if ( {_cursTarget isKindOf _x} count ["LandVehicle","Air"] > 0 && (mps_player_class == "engineer" || !mps_class_limit) ) then { _vehicle_repair = _cursTarget addAction [format[localize "STR_Client_Repair_menu",_type],(mps_path+"action\mps_vehicle_repair.sqf"),[],-1,true,true,"",""]; };
...
*/
// Question Civillians
			if ({_cursTarget isKindOf _x} count ["Man"] > 0 && side _cursTarget == civilian && alive  _cursTarget) then { _intelaction1 = _cursTarget addAction [format["Ask %1 for task INTEL",_type], (mps_path+"action\mps_interaction_question.sqf"),[],97,true,true,"",""]; };
			if ({_cursTarget isKindOf _x} count ["Man"] > 0 && side _cursTarget == civilian && !alive _cursTarget) then { _intelaction2 = _cursTarget addAction [format["Search %1 for task INTEL",_type], (mps_path+"action\mps_interaction_question.sqf"),[],96,true,true,"",""]; };
