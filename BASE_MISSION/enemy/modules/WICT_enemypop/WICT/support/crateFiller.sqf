...
// by Tophe of Östgöta Ops, adapted for WICT and Dedicated server by ArmAIIholic
// Usage:
// null = [this] execVM "WICT\support\crateFiller.sqf"
//
// Optional settings:
// null = [this,side(west,east,all),game(all,arma,oa,baf,pmc,user),refill(true/false),weapons(amount),magazines(amount),equipment(amount)] execVM "WICT\support\crateFiller.sqf"
//
// Default values:
...
while {_run} do {	
	...
	// ALL
			// EQUIPMENT
				{ 
					[nil,_nil,rADDWEAPONCARGO,_box,_x,_amounte] call RE; 
				}
				foreach _equipment;
			...
			// ORDONANCE 
				{ 
					[nil,_nil,rADDMAGAZINECARGO,_box,_x,_amountm] call RE; 
				}
				foreach _ordonance;
				if (_oa || _baf) then {
					{ 
	...
	// WEST
			// ORDONANCE 
				...
				if (_west && _oa) then {
					{ 
						[nil,_nil,rADDMAGAZINECARGO,_box,_x,_amounte] call RE; 
					}
			...
			// WEAPONS & MAGAZINES
				if (_west && _arma) then {
					{ 
						[nil,_nil,rADDWEAPONCARGO,_box,_x,_amountw] call RE; 
					}
