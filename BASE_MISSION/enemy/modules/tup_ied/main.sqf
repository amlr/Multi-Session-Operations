	...
	if (({(getpos _x distance _pos) < _size} count ([] call BIS_fnc_listPlayers) == 0) && ((_pos distance getmarkerpos "ammo") > 250) && ((_pos distance getmarkerpos "ammo_1") > 250)) then {		
		...
		if (_fate < tup_ied_threat) then {
			...
			if (tup_ied_enemy == 1) then {
				...
				_trg setTriggerStatements["this && ({(vehicle _x in thisList) && ((getposATL _x) select 2 < 75)} count ([] call BIS_fnc_listPlayers) > 0) && ({side _x == EAST} count (thisList) > 2)",	format ["null = [getpos (thisTrigger),%1] execvm 'enemy\modules\tup_ied\Ambient_IED.sqf'",_size], ""]; // for repeated = format ["null = [getposATL (thisTrigger),%1] execvm 'enemy\modules\tup_ied\Remove_IED.sqf';"
			} else {
