...
mando_mapfallout = 
{
   ...
   switch (_event) do
   {
      case "onLoad":
      {
         ...
         (_display displayCtrl 100) ctrlSetText ("scripts\nuke\raptor6_mfd1.paa");

		mando_echo_zones = [];
		for [{_i=0},{_i<50},{_i=_i+1}] do
		{
			_var = ECHO_FalloutGL getVariable format["%1", _i];
			if (!isNil "_var") then
			{
				_fallout_zone = createLocation["Name", [(_var select 3) select 0, (_var select 3) select 1], _var select 4, _var select 5];
				_fallout_zone setDirection (_var select 6);
				_mando_echo_zones = _mando_echo_zones + [_fallout_zone];
			};
		};
