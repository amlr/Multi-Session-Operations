...
while {true} do
{
	...
	if(player != driver _plane && !isNil "_var") then
	{
		if(!LDL_ac130_active) then
		{
			...
			if(_copilot) then
			{
				...
				{
					_action1 = _plane addAction ["<t color=""#FA1845"">"+"AC130 Camera", "support\modules\rmm_cas\LDL_ac130\Actions\ac130_action_copilot.sqf",[2], 99, false, true];
				};
				
				if(LDL_inGameActions select 3) then
				{	
					_action2 = _plane addAction ["<t color=""#FA1845"">"+"Activate LDL-Systems", "support\modules\rmm_cas\LDL_ac130\Actions\ac130_action_coop_copilot.sqf",[2], 99, false, true];
	...
	if(player == driver _plane && !isNil "_var") then
	{
		if(!LDL_ac130_active) then
		{
			...
			{
				_action1 = _plane addAction ["<t color=""#FA1845"">"+"AC130 Autopilot", "support\modules\rmm_cas\LDL_ac130\Actions\ac130_action_pilot.sqf",[1], 99, false, true];
			};
			
			if(LDL_inGameActions select 3) then
			{
				_action2 = _plane addAction ["<t color=""#FA1845"">"+"Activate LDL-Systems", "support\modules\rmm_cas\LDL_ac130\Actions\ac130_action_systems_pilot.sqf",[1], 99, false, true];
			...
			while{((player == driver _plane) && (player in _plane))} do
			{
				...
				if(LDL_SystemsActivated) then
				{
					...
					_action2 = _plane addAction ["<t color=""#FA1845"">"+"Deactivate LDL-Systems", "support\modules\rmm_cas\LDL_ac130\Actions\ac130_action_systems_pilot.sqf",[1], 99, false, true];
