...
_act_id_move_cow = gc_pack_cow  addaction ["Move Cow", "support\modules\gc_pack_cow\move_cow.sqf", "", 0, false, true, "", ""];
_act_id_stop_cow = gc_pack_cow  addaction ["Stop Cow", "support\modules\gc_pack_cow\stop_cow.sqf", "", 0, false, true, "", ""];
...
while {alive gc_pack_cow} do 
	...
	if (_distance < 5) then 
		...
		if (_act_id_deploy_crate == 9999) then 
			{
			_act_id_deploy_crate = gc_pack_cow addaction ["Deploy Ammo Crate", "support\modules\gc_pack_cow\deploy_crate.sqf", "", 0, false, true, "", ""];
			};
		};
	...
	if (_distance > 5) then 
		...
		if (_act_id_load_crate == 9999) then 
			{
			_act_id_load_crate = gc_pack_cow addaction ["Load Ammo Crate", "support\modules\gc_pack_cow\load_crate.sqf", "", 0, false, true, "", ""];
			};
		};
