if (local _this) then {
	...
	_this setvariable ["revive_eh_killed", _this addeventhandler ["killed", "if (player == (_this select 0)) then {[] spawn {waituntil {alive player;}; player call revive_fnc_init;};};"]];
};