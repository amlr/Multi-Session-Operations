...
nul0 = [_IEDhost,_beep] execVM "enemy\modules\SR5_IEDdetect\IEDdetect_attach.sqf";
  ...
  if (_armed == 1) then {
  	...
  	for [{_y = 0},{_y < (count _nearUnits)},{_y = _y + 1}] do {
    	...
    	if (side _soldier == _enemySide) then {
    		_fearing = 1;
        	nul0 = [_soldier,_triggerman] execVM "enemy\modules\SR5_IEDdetect\IEDdetect_cursorTarget.sqf";
    	} else {
    		_fearing = 0;
    	};
  	};
