...
while {alive _soldier} do {
  ...
  if (_fate < _chance) then {
    ...
    if (count _goodSpots > 0 && ((getPos _soldier) select 2 < 5)) then {
      ...
      _IEDpos = [(_IEDpos select 0) + _rndX, (_IEDpos select 1) + _rndY, 0];
      ...
      _proximityIED setPosATL (position _proximityIED);
      ...
      if (_rnd < _typeRate) then {
        nul0 = [_proximityIED,_beep,0] execVM "enemy\modules\SR5_IEDdetect\IEDdetect_proximity.sqf";
      } else {
        nul0 = [_proximityIED,_beep,1] execVM "enemy\modules\SR5_IEDdetect\IEDdetect_proximity.sqf";
      };
