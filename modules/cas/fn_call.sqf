if (RMM_cas_lastTime + RMM_cas_frequency < time) then {
	...
	hintcadet format ["%1 requested to %2 by %3", call (RMM_cas_lines select 2) select (lbCurSel 2), RMM_cas_lines select 0 select (lbCurSel 0), RMM_cas_lines select 1 select (lbCurSel 1)];
