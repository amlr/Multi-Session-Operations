...
private ["_debug","_airports","_destairfield","_endpoints","_helilandings","_center","_helidest","_planedest","_planelandings","_destinations"];
...
_airports = ["Airport"];
_endpoints = [[0,10000,1000],[10000,10000,1000],[10000,0,1000],[0,0,1000]]; // corners of maps for aircraft to fly away to
_destairfield = [];
_helilandings = [];
_helidest = [];
_planedest = [];
...
_planelandings = ["Land_Hangar_2","Land_SS_hangar","Land_SS_hangarD","Land_Mil_hangar_EP1"];
{
    _center = position _x;
    {
        _planedest = _planedest + (_center nearObjects [_x , 1500]);
    } forEach _planelandings;
...
{
    _helidest = _helidest + (_center nearObjects [ _x , 20000]);
} forEach _helilandings;
...
for "_j" from 0 to (_destinations-1) do {
    ...
    [_j, _helidest, _planedest, _debug, _endpoints] spawn {
        ...
        while{true} do {
            ...
            _endpos = (_endpoints select (random 3));
            ...
            // Count factions at destination for dominant side
            {
                ...
                _vehiclelist =  [0, _facs,_front] call "modules\AirTraffic\fnc_findVehicleType2.sqf"; 
                
                if (count _vehiclelist > 0) then {
                    _vehicle = (_vehiclelist) call BIS_fnc_selectRandom;
                } else {
                    _facs = ["BIS_TK_CIV","BIS_CIV_special","CIV", "CIV_RU"];
                    _vehicle =  ([0, _facs,_front] call "modules\AirTraffic\fnc_findVehicleType2.sqf") call BIS_fnc_selectRandom;
                    _airfieldside = civilian;
                    if (_debug) then {
