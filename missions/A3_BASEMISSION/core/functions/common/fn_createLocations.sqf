...
_objs = [_ctypes,[],CRB_LOC_DIST,_debug,"ColorGreen","mil_dot"] call mso_core_fnc_findObjectsByType;
...
// If no other locations nearby, for each object check how many objects in the area, set size
{
	...
	if ((count _nearlocs) == 0) then 
	{
		...
		_tempObjs = [_ctypes,[_x], 400,_debug,"ColorRed","mil_dot"] call mso_core_fnc_findObjectsByType;
...
// StrongpointArea - Find towns, cities, find roads into towns, find flatempty space, 4 for each town/city
{
		...
		{		
			...
			//[_t, _x, "Icon", [0.5,0.5], "TYPE:", "mil_dot", "COLOR:", "ColorBlack", "GLOBAL"] call CBA_fnc_createMarker;
			_position = _x;
			_currentPos = [0,0,0];
			_done = false;
			_loop = 0;
