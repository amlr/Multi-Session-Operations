        ...
        if (_debug) then {
			...
			for "_i" from 0 to ((count CQBpositionsRegLocal) - 1) do {
         		_t = format["rp%1",_i];
    			_m = [_t, position ((CQBpositionsRegLocal select _i) select 0), "Icon", [1,1], "TYPE:", "mil_dot", "COLOR:", "ColorRed"] call CBA_fnc_createMarker;
    		};
            
            _i = 0;
			for "_i" from 0 to ((count CQBpositionsStratLocal) - 1) do {
         		_t = format["sp%1",_i];
    			_m = [_t, position ((CQBpositionsStratLocal select _i) select 0), "Icon", [1,1], "TYPE:", "mil_dot", "COLOR:", "ColorGreen"] call CBA_fnc_createMarker;
    		};
		};
