...
if(isNil "twnmgr_broadcastMP")then{twnmgr_broadcastMP = 1;};
...
CRB_updateDetectedMarker = {
	private ["_name","_pos","_detector","_detected","_color","_detectorTxt","_globalmsg","_acttext"];
	...
	_acttext = """%1_mgr"" setMarkerColor ""%2"";";
	if (twnmgr_broadcastMP == 1) then {
		_acttext = _acttext + " [-1, {[%3,""HQ""] sideChat ""%4""}] call CBA_fnc_globalExecute;";
	};
	...
	if(_detector == civilian) then {
		...
		format[_acttext, _name, _color, playerSide, _globalmsg];     
	} else {
		...
		format[_acttext, _name, _color, _detector, _globalmsg];		   
	};		
};

CRB_updateSeizedMarker = {
	private ["_name","_detector","_color","_detectorTxt","_globalmsg","_acttext"];
	_name = _this select 0;
