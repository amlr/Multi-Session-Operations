class RMM_ui_settings {
	...
	class controls {
		...
		class VDSlider : CUI_Slider {
			idc = 2;
			y = CUI_Box_Row(0,2.5);
			onSliderPosChanged = "setviewdistance round(_this select 1); ctrlSetText [1,format['%1/%2 m', viewdistance, settings_maxvd]]; player setvariable ['viewdistance', viewdistance, true];";
		};
