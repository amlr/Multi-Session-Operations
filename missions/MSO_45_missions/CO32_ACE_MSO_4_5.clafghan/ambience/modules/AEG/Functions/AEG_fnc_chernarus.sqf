/*
    ...
    Execution:
    Loaded into memory by AEG_chernarus.sqf.  Function called by AEG_events.sqf if a power related event has occurred on Chernarus.
*/
...
// Determine which which power plant or transformer bus is being affected by a power related event based on the public variable that has changed.
    switch (_var) do
    {
        ...
        case "AEG_on_P_Elek":
        {
                // Determine whether this nodes feeds power to street lamps.
                _feeds = false;

                // Determine what sounds effects are associated with this node.
                _effect = "PLANT";

                // Determine which object this is.
                _name = "P_Elek";

                // If this is a power plant, determine which smokestack is associated with it.
                _stack = "S_Elek";

                // Create an array of zones that this objects feeds/distributes power to if applicable.
                _zones = [];
