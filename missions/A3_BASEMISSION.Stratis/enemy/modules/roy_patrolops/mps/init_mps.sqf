...
// Declare a mission name and credits
	...
	mps_credits = "By [OCB]EightySix Ported by TAW_Zorrobyte";
...
// Enable A2, Arrowhead, BAF, PMC Content
	mps_a2	= false; 	// Set false to turn off A2 content
	mps_oa	= false; 	// Set false to turn off OA content
	mps_baf	= false; 	// Set false to turn off BAF content
	mps_pmc	= false; 	// Set false to turn off PMC content
	mps_aaw	= false; 	// Set false to turn off AAW content
	mps_a3	= true; 	// Set false to turn off A3 content

	mps_co	= false; 	// Set false to turn off the use of advanced code of ArmA 2 Combined opertions with patch 1.60
...
// Faction list can be found in the configuration files to choose from which ones to use.
	...
	SIDE_A = [west,"BLU_F"];		// Player Side
	SIDE_B = [east,"OPF_F"];	// Enemy Side
	SIDE_C = [east,"OPF_F"];	// Insurgent Side
...
// =========================================================================================================
	...
	mps_params_armour    = [0,3];    // [min,max] number of armour    vs. player number
    	...
    	mps_params_inf        = [4,10];    // [min,max] number of infantry    vs. player number
...
// File path folder
	mps_path = "enemy\modules\roy_patrolops\mps\";
...
// Dynamic BIS Object spawner *removed since not in A3*
...
// Calling Team killing :) HAHAHAHA!!!!1
