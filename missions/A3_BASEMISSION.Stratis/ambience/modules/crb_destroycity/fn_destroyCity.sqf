...
//	_pos = [(_pos select 0) + (_bc select 0), (_pos select 1) + (_bc select 2), (_pos select 2) + (_bc select 1)];
	...
	if (_seedLocal < 0.5) then {
		...
		if (_debug) then {_marker = createMarker ["X" + str _posTotal, position _x]; _marker setmarkertype "mil_dot"; _marker setmarkercolor "colorred";};
	} else {
		if (_seedLocal > 0.9) then {
			//_x hideobject true;
			_x setdamage 1;
			if (_debug) then {_marker = createMarker ["X" + str _posTotal, position _x]; _marker setmarkertype "mil_dot"; _marker setmarkercolor "colorblue";};
                        if(_fire && !isDedicated && !isHC) then {
				_pos set [2, -1];
                                _centre = "Sign_Pointer_F" createVehicleLocal _pos;
				_centre setPos _pos;
				0 = [_centre, exp((_size select 0) * (_size select 1) * (_size select 2) / 500) min 10, time, false, false] spawn BIS_Effects_Burn;
                        };
		} else {
			_x setdamage 0.5;
			if (_debug) then {_marker = createMarker ["X" + str _posTotal, position _x]; _marker setmarkertype "mil_dot"; _marker setmarkercolor "colorgreen";};
		};
	};
} foreach _buildings;
