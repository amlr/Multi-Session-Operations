...
tup_airtraffic_getHeliports = {
	...
	_helilandings = [
	    //A2 "HeliH","HeliHRescue","HeliHCivil"
        //A3
        "Land_HelipadEmpty_F","Land_HelipadSquare_F"
    ];
	([_helilandings, [], _mapsize * 1.4, tup_airtraffic_debug,"ColorBlack","mil_dot"] call mso_core_fnc_findObjectsByType);
};
	...
	[_j, _helidest, _planedest, _mapsize] spawn {
		...
		while {true} do {
			...
			if (typeof _aircraftVehicle == "MV22") then {
				...
				_landEnd = "Land_HelipadEmpty_F" createVehicle _mv22pos;
			};
