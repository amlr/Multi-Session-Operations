...
CRBPROFILERSTART("mso_core_fnc_createCompositionE")

_script = _this select 0;
_azi 	= _this select 1;
_pos 	= _this select 2;
_objs = [];
//not in A3 _objs = call (compile (preprocessFileLineNumbers format ["\CA\modules_e\DynO\data\scripts\compositions\%1.sqf",_script]));
private ["_posX", "_posY"];
_posX = _pos select 0;
_posY = _pos select 1;
_newObjs = [];
